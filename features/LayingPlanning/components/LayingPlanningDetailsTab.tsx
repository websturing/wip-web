...
export const LayingPlanningDetailsTab = ({
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl font-bold text-zinc-900">
                  {editingDetail
                    ? 'Edit Laying Detail'
                    : 'Add New Laying Detail'}
                </h3>
                <p className="text-sm text-zinc-500 font-medium">
                  Configure marker code, yards, and size ratio allocations.
                </p>
              </div>
            </div>
            <Button
              ...
              className="h-10 w-10 p-0 rounded-full text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100"
            >
  ...
  const filteredDetails = details.filter((detail: any) => {
    ...
    return (
      detail.marker_code?.toLowerCase().includes(q) ||
        ...
        <div className="bg-white border border-zinc-100 rounded-[2rem] shadow-2xl w-full max-w-lg flex flex-col animate-in zoom-in-95 duration-300">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-500 font-medium">
                  {warehouseDetails.length > 1
                    ? `Multiple Tables (${warehouseDetails.length})`
                    : `Table ${tableNumbers[0]} • ${warehouseDetails[0].marker_code}`}
                </p>
              </div>
            </div>
            <Button
              type="button"
              variant="ghost"
              onClick={() => setWarehouseDetails(null)}
              className="h-10 w-10 p-0 rounded-full text-zinc-400 hover:text-zinc-600 hover:bg-zinc-100"
            >
              <Icon icon="solar:close-circle-bold" className="w-6 h-6" />
            </Button>
          ...
          <div className="p-6 md:p-8 space-y-8">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-5 bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100/50 rounded-2xl shadow-sm">
                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-400/80 mb-1 flex items-center gap-1.5">
                  <Icon
              ...
              <div className="p-5 bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100/50 rounded-2xl shadow-sm">
                <p className="text-[10px] font-black uppercase tracking-widest text-blue-400/80 mb-1 flex items-center gap-1.5">
          ...
          <div className="p-6 md:p-8 border-t border-zinc-100 flex items-center justify-end gap-3 bg-zinc-50/50 rounded-b-[2rem]">
            <Button
              variant="ghost"
              onClick={() => setWarehouseDetails(null)}
              className="h-12 px-6 font-bold text-zinc-500 hover:text-zinc-800 hover:bg-zinc-200 rounded-xl transition-colors"
            >
              Cancel
            </Button>
            <Button
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-zinc-900/40 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
          ...
          <p className="text-sm text-zinc-500 mb-8">
            Are you sure you want to delete this laying detail? This action
            cannot be undone.
          </p>
          ...
          <div className="flex items-center gap-3 w-full">
            <Button
              ...
              className="flex-1 h-11 font-bold text-zinc-600 bg-zinc-100 hover:bg-zinc-200 rounded-xl"
            >
              Cancel
            </Button>
            <Button
              ...
              className="flex-1 h-11 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl shadow-lg shadow-red-500/30"
            >
  ...
  const renderToast = () => {
    ...
    return createPortal(
      <div className="fixed bottom-6 right-6 z-[9999] animate-in slide-in-from-bottom-5 fade-in duration-300">
        <div
          className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl border ${toastMessage.type === 'success' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-red-50 border-red-100 text-red-700'}`}
        >
          <Icon
            icon={
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-zinc-100 pb-4 gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            ...
            <Button
              onClick={() => {
                const selected = details.filter((d) =>
                  selectedDetailIds.includes(d.id),
                )
          ...
          <Button
            onClick={handleAdd}
            className="h-10 px-5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-500/30 transition-all hover:-translate-y-0.5 w-full sm:w-auto shrink-0"
          >
            <Icon
      ...
      {filteredDetails.length > 0 && viewMode === 'card' && (
        <div className="flex items-center px-2 pb-1">
          ...
          >
            <div
              className={`w-5 h-5 rounded flex items-center justify-center border transition-all ${selectedDetailIds.length > 0 && selectedDetailIds.length === filteredDetails.length ? 'bg-indigo-600 border-indigo-600' : 'bg-zinc-50 border-zinc-300 group-hover:border-indigo-400'}`}
            >
              {selectedDetailIds.length > 0 &&
            ...
            <span className="text-sm font-bold text-zinc-600 group-hover:text-zinc-900 transition-colors">
              Select All ({filteredDetails.length})
            </span>
          </label>
        </div>
      )}
        ...
        {filteredDetails.length === 0 ? (
                  ...
                  <tr className="bg-zinc-50 border-b border-zinc-200 text-zinc-500 font-semibold uppercase tracking-wider text-[11px]">
                    <th className="px-4 py-3 w-12 text-center">
                      <button
                        ...
                        className={`w-5 h-5 rounded flex items-center justify-center border mx-auto transition-all ${selectedDetailIds.length > 0 && selectedDetailIds.length === filteredDetails.length ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-zinc-300 hover:border-indigo-400'}`}
                      >
                        {selectedDetailIds.length > 0 &&
                ...
                <tbody className="divide-y divide-zinc-100">
                  {filteredDetails.map((detail: any, index: number) => {
                    ...
                    return (
                      <tr
                        key={detail.id}
                        className={`hover:bg-zinc-50/80 transition-colors ${isSelected ? 'bg-indigo-50/30' : ''}`}
                      >
                        <td className="px-4 py-4 text-center">
                          <button
                            onClick={() => toggleSelect(detail.id)}
                            className={`w-5 h-5 rounded flex items-center justify-center border mx-auto transition-all ${isSelected ? 'bg-indigo-600 border-indigo-600' : 'bg-white border-zinc-300 hover:border-indigo-400'}`}
                          >
                            {isSelected && (
                              <Icon
                                icon="solar:check-read-bold"
                        ...
                        <td className="px-4 py-4">
                          <div className="flex flex-col gap-0.5">
                            <span className="text-zinc-900 font-bold">
                              {markerLengthYard.toFixed(2)} YD
                            </span>
                            <span className="text-xs text-zinc-400 font-medium">
                              {detail.marker_yard} YD {detail.marker_inch}" (A:{' '}
                              {detail.allowance_inch}")
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-4">
                          <span className="text-indigo-600 font-bold">
                            {totalYard.toFixed(2)} YD
                          </span>
                        </td>
                        <td className="px-4 py-4">
                          <div className="flex flex-wrap gap-1 max-w-xs">
                            {detail.sizes?.map((sz: any) => (
                              <span
                                key={sz.id}
                                className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-zinc-50 border border-zinc-200 rounded text-[10px] font-bold text-zinc-700"
                              >
                                ...
                                >
                                  ...
                                  <div className="space-y-3 max-h-64 overflow-y-auto pr-1 custom-scrollbar">
                                    {detail.materials.map(
                                      (mat: any, idx: number) => {
                                        const typeName =
                                          mat.detail_type || 'Material'
                                        const colorName = mat.color_name
                                        const fabricName = mat.fabric_content
                                        return (
                                          <div
                                            key={idx}
                                            className="bg-zinc-50/80 rounded-xl p-3 border border-zinc-100"
                                          >
          ...
          filteredDetails.map((detail: any, index: number) => {
            ...
            return (
              <div
                key={detail.id}
                className={`group p-5 bg-white border ${isSelected ? 'border-indigo-400 ring-2 ring-indigo-500/10' : 'border-zinc-100 hover:border-blue-200'} rounded-2xl shadow-[0_2px_10px_-3px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_20px_-6px_rgba(0,0,0,0.08)] transition-all duration-300`}
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className="flex items-center h-12">
                      <button
                        onClick={() => toggleSelect(detail.id)}
                        className={`w-6 h-6 rounded-md flex items-center justify-center border transition-all ${isSelected ? 'bg-indigo-600 border-indigo-600' : 'bg-zinc-50 border-zinc-300 hover:border-indigo-400'}`}
                      >
                        {isSelected && (
                          <Icon
                            icon="solar:check-read-bold"
                        ...
                        return (
                          <div className="flex flex-wrap items-center gap-3 text-[13px] text-zinc-500 font-medium">
                            <span className="flex items-center gap-1.5">
                              ...
                              />{' '}
                              {detail.layer_qty} Layers
                            </span>
                            <span className="w-1 h-1 bg-zinc-300 rounded-full" />
                            <span>
                              {detail.marker_yard} YD {detail.marker_inch}"
                            </span>
                            <span className="w-1 h-1 bg-zinc-300 rounded-full" />
                            ...
                            <span
                              ...
                              title="Marker Length = Yard + (Inch/36) + (Allowance/36)"
                            >
                            ...
                            <span
                              ...
                              title="Total Yard = Marker Length × Layer Qty"
                            >
                            ...
                            {detail.materials?.length > 0 && (
                              <>
                                ...
                                <Popover.Root>
                                  <Popover.Trigger asChild>
                                    <button
                                      className="flex items-center gap-1 text-emerald-600 font-bold hover:text-emerald-700 transition-colors outline-none focus:ring-2 focus:ring-emerald-500/20 rounded"
                  ...
                  <div className="flex flex-col items-end gap-3 shrink-0">
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button
                        onClick={() => handleEdit(detail)}
                        variant="ghost"
                        className="h-8 w-8 p-0 text-blue-600 hover:bg-blue-50 border border-transparent hover:border-blue-100 rounded-lg"
                      >
                        <Icon
                          icon="solar:pen-bold-duotone"
                          className="w-4 h-4"
                ...
                {detail.sizes && detail.sizes.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-zinc-100 flex flex-wrap gap-2">
                    {detail.sizes.map((sz: any) => (
                      <div
                        key={sz.id}
                        className="inline-flex items-center gap-2 px-2.5 py-1.5 bg-zinc-50 border border-zinc-100 rounded-lg text-xs font-bold text-zinc-700"
                      >
                        <span className="text-zinc-400">
                          {sz.size_name || 'Size'}:
                        </span>
