...
export const Packing = () => {
    ...
    const { data: packings, isLoading, refresh } = usePacking({ date: viewDate });
    ...
    // Grouping logic (Packing has no line)
    const groupedData = useMemo(() => {
        ...
        return groups;
    }, [packings, filterGL, filterLot, formatGL]);
    ...
    if (isLoading) return (
        <div className="p-20 flex flex-col items-center justify-center gap-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Loading Packing Data...</span>
        </div>
    );
            ...
            <div className="p-8 max-w-[1600px] mx-auto">
                <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-12">
                    <div className="flex items-center gap-5">
                        <div className="bg-emerald-900 p-4 rounded-[1.5rem] text-white shadow-2xl">
                            <Icon icon="solar:box-bold-duotone" className="w-8 h-8" />
                        </div>
                        <div>
                            <h2 className="text-2xl font-black text-zinc-900 tracking-tight uppercase">Packing Feed</h2>
                            <div className="flex items-center gap-2 mt-1">
                                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                                <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest">Finished Goods Processing</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-[2rem] border border-zinc-100 shadow-sm">
                        ...
                        <Button
                            onClick={() => router.push('/admin/packing/create')}
                            className="bg-emerald-900 hover:bg-emerald-800 text-white rounded-2xl px-6 h-10 flex items-center gap-2 transition-all shadow-md active:scale-95 text-xs font-bold"
                        >
                ...
                <div className="w-full">
                    {Object.keys(groupedData).length === 0 ? (
                        <div className="py-32 bg-zinc-50 rounded-[3rem] border border-dashed border-zinc-200 flex flex-col items-center justify-center text-zinc-400">
                            ...
                            <p className="text-xs font-black uppercase tracking-[0.2em]">No packing data found</p>
                        </div>
                    ) : viewMode === 'cards' ? (
                                    ...
                                    >
                                        <div className="flex items-center justify-between w-full mb-6">
                                            <div className="flex items-center gap-3">
                                                <div className="w-12 h-12 rounded-2xl bg-emerald-900 text-white flex flex-col items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                                        ...
                                        <div className="flex items-center justify-between w-full p-4 bg-white rounded-2xl border border-zinc-100 shadow-sm gap-2">
                                            ...
                                            <div className="flex flex-col items-end pl-4 flex-1">
                                                <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-1 text-right">Total Out</span>
                                                <span className="text-sm font-black text-emerald-600">{data.total_out}</span>
                                            </div>
                                        </div>
                                    </button>
                                    ...
                                    {expandedCards[key] && (
                                        <div className="p-4 space-y-3 animate-in fade-in slide-in-from-top-4 duration-500 bg-white">
                                            {data.entries.map((p: any, pIdx: number) => (
                                                <div key={pIdx} className="bg-zinc-50/50 rounded-2xl border border-zinc-100 p-4 hover:border-emerald-200 transition-all group/entry">
                                                    <div className="flex items-center justify-between mb-3 pb-3 border-b border-zinc-100/50">
                                                        ...
                                                        <div className="bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100/50">
                                                            <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">{p.color}</span>
                                                        </div>
                                                    </div>
                                                    <div className="flex gap-6">
                                                        ...
                                                        <div className="flex-1">
                                                            <div className="flex justify-between mb-1.5">
                                                                <span className="text-[8px] font-black text-emerald-400 uppercase tracking-tighter">Done OUT</span>
                                                                <span className="text-[11px] font-black text-emerald-600">{p.qty_out}</span>
                                                            </div>
                                                            <div className="h-1 bg-emerald-100/50 rounded-full overflow-hidden">
                            ...
                            <table className="w-full border-collapse">
                                <thead>
                                    <tr className="bg-zinc-50/50 border-b border-zinc-100">
                                        ...
                                        <th className="px-6 py-5 text-left text-[10px] font-black text-zinc-400 uppercase tracking-widest">Colors</th>
                                        <th className="px-6 py-5 text-right text-[10px] font-black text-zinc-400 uppercase tracking-widest">Total Qty</th>
                                        <th className="px-6 py-5 text-center text-[10px] font-black text-zinc-400 uppercase tracking-widest">Actions</th>
                                    </tr>
                                </thead>
                                            ...
                                            <tr key={key} className="hover:bg-zinc-50/30 transition-colors group">
                                                ...
                                                <td className="px-6 py-8">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <div className="bg-emerald-900 text-white text-[10px] font-black px-2 py-0.5 rounded shadow-sm">
                                                                {data.lotClean}
                                                            </div>
                                                ...
                                                <td className="px-6 py-8">
                                                    <div className="flex flex-wrap gap-1">
                                                        {allColors.map((c: any, i: number) => (
                                                            <span key={i} className="bg-emerald-50 text-emerald-600 text-[9px] font-black px-2 py-0.5 rounded border border-emerald-100 uppercase tracking-widest">
                                                                {c}
                                                            </span>
                                                        ))}
                                                    </div>
                                                ...
                                                <td className="px-6 py-8 text-right">
                                                    <div className="flex flex-col items-end">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-[9px] font-black text-emerald-400 uppercase">OUT:</span>
                                                            <span className="text-base font-black text-emerald-600 tracking-tight">{data.total_out}</span>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-8 text-center">
                                                    <button
                                                        ...
                                                        className="w-10 h-10 rounded-xl bg-zinc-50 hover:bg-emerald-900 hover:text-white border border-zinc-100 transition-all flex items-center justify-center text-zinc-400 shadow-sm mx-auto group-hover:scale-110 active:scale-95"
                                                    >
                                ...
                                <div className="h-full flex flex-col">
                                    <div className="p-10 border-b border-zinc-100 bg-zinc-50/50 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="w-14 h-14 bg-emerald-900 rounded-2xl flex flex-col items-center justify-center text-white shadow-xl">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em] mt-1">PACKING LOGS SUMMARY</p>
                                            </div>
                                        </div>
                                        <button
                                            onClick={() => setSelectedGroupKey(null)}
                                            className="w-12 h-12 rounded-2xl bg-white border border-zinc-100 hover:bg-emerald-900 hover:text-white flex items-center justify-center text-zinc-400 transition-all shadow-sm active:scale-95 outline-none"
                                        >
                                    ...
                                    <div className="flex-1 overflow-y-auto p-10 space-y-4 font-sans no-scrollbar">
                                        {groupedData[selectedGroupKey].entries.map((p: any, pIdx: number) => (
                                            <div key={pIdx} className="bg-zinc-50/50 rounded-3xl border border-zinc-100 p-6 hover:border-emerald-100 transition-colors">
                                                <div className="flex flex-col gap-6 mb-6 pb-6 border-b border-zinc-100">
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        <div className="px-4 py-1.5 bg-emerald-50 border border-emerald-100 rounded-full">
                                                            <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">{p.color}</span>
                                                        </div>
                                                    </div>

                                                    <div className="space-y-3">
                                                        <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block px-1">Size Breakdown</span>
                                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                                            {p.item_details?.map((d: any, dIdx: number) => (
                                                                <div key={dIdx} className="bg-white border border-zinc-100 p-3 rounded-2xl flex flex-col items-center shadow-sm">
                                                                    ...
                                                                    <span className="text-sm font-black text-emerald-600 leading-none">{d.qty_output}</span>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="flex items-center justify-between">
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1 leading-none">Yield IN</span>
                                                    ...
                                                    <div className="flex flex-col items-end">
                                                        <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1 leading-none">Finished OUT</span>
                                                        <span className="text-xl font-black text-emerald-600 leading-none">{p.qty_out} <span className="text-[10px] font-bold text-emerald-200 italic">PCS</span></span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
